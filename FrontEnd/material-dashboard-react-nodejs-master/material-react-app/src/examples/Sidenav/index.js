...
function Sidenav({ color, brand, brandName, routes, ...rest }) {
  ...
  const { role } = useContext(AuthContext); // 👈 role từ backend
  ...
  /* ================== FILTER ROUTES THEO ROLE ================== */
  const allowedRoutes = routes.filter((route) => {
    if (!route.roles) return true; // không khai báo => ai cũng thấy
    ...
    >
      ...
      <MDBox pt={3} pb={1} px={4} textAlign="center">
        <MDBox
          ...
          sx={{ cursor: "pointer" }}
        >
          <Icon fontWeight="bold">close</Icon>
        </MDBox>
